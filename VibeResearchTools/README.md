...
- `case`: inspect one datapack with prediction top-k, GT, injection metadata and input file summaries.
...
These tools may read labels and injection metadata for offline analysis. Algorithm code under `algorithms/evidencerank` must not read those files.
