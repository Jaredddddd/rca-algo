...
FEATURE_FAMILIES = {
    "metric_magnitude": frozenset({
        "metric_max_z", "metric_mean_z", "metric_anomaly_count", "metric_value_delta",
    }),
    "availability_drop": frozenset({
        "metric_count_drop_shift", "trace_count_drop_shift",
    }),
    "trace_latency": frozenset({
        "trace_duration_z", "trace_duration_delta", "trace_self_duration_relative_shift",
    }),
    "trace_traffic": frozenset({
        "trace_count_delta", "trace_count_rise_shift",
    }),
    "trace_protocol": frozenset({
        "trace_endpoint_shift", "trace_error_rate", "trace_status_code_shift",
    }),
    "log_evidence": frozenset({
        "log_count_delta", "log_error_rate", "log_template_delta",
    }),
    "observability_volume": frozenset({
        "abnormal_metric_rows", "abnormal_trace_rows",
    }),
    "topology_context": frozenset({
        "topology_in_degree", "topology_out_degree",
    }),
}
...
def _calibrated_feature_weights() -> dict[str, float]:
    feature_to_family = {
        feature: family
    ...
    return {
        feature: SEMANTIC_FEATURE_PRIOR[feature]
        * CALIBRATED_FAMILY_MULTIPLIERS[feature_to_family[feature]]
        for feature in BASE_FEATURE_NAMES
    }


FEATURE_WEIGHTS = _calibrated_feature_weights()
