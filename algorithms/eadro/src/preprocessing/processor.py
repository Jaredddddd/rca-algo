...
class Processor(DataProcessor):
    ...
    def create_metadata_for_folder(self, dataset_folder: str, label: str) -> DatasetMetadata:
        if label == "train":
            ...
            # Test data must reuse train metadata for consistent feature dimensions
            metadata = DatasetMetadata.from_pkl(
                str(
                    Path(self.config.get("paths.metadata"))
                    / f"{self.dataset}_metadata.pkl"
                )
            )
            if metadata is None:
                raise ValueError(
                    f"Train metadata not found at "
                    f"{self.config.get('paths.metadata')}/{self.dataset}_metadata.pkl. "
                    f"Please preprocess train data before test data."
                )
            return metadata
