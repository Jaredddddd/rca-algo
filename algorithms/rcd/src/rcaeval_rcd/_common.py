from rcabench_platform.v2.algorithms.spec import AlgorithmArgs, AlgorithmAnswer
from rcabench_platform.v2.logging import timeit

from rcabench_platform.v2.graphs.sdg.build_.rcaeval import load_inject_time as rcaeval_load_inject_time
from rcabench_platform.v2.graphs.sdg.build_.rcabench import load_inject_time as rcabench_load_inject_time
...
@timeit()
def load_simple_metrics(dataset: str, input_folder: Path) -> list[pd.DataFrame]:
    ...
    if is_rcabench_like_dataset(dataset, input_folder):
        ...
        output = []
        for metrics in [normal_metrics, abnormal_metrics]:
            ...
            lf = metrics
            df = convert_simple_metrics(lf).to_pandas()
            output.append(df)
        return output
...
def convert_simple_metrics(lf: pl.LazyFrame) -> pl.DataFrame:
    ...
    lf = lf.with_columns(
        pl.col("metric").replace(list(RENAME_METRICS.keys()), list(RENAME_METRICS.values()), default=pl.col("metric"))
    )
