...
- Accept `CREST15_ACCEPTED` as the new default `crest`.
- There are 18 `improved_to_hit1` cases and no `regressed_from_hit1` cases.
- The general mechanism is upstream cluster arbitration: a near-tie candidate is
  promoted only when it has stronger trace mutation than multiple incoming
  callers whose propagation burden is higher, with CREST14 mutation arbitration
  retained as a narrow fallback.
- The version still does not solve the overall `AC@1 >= 0.85` goal.
