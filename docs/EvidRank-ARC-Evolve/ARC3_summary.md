# EvidenceRank ARC3 Summary

- Created: 2026-06-03T18:04:45+08:00
- Source: `ARC3`
...
- ARC3 keeps ARC2's ranking behavior but removes explicit fixed blend constants.
- Reliability correction is now derived as `1 / active_family_count`; contrast correction is derived as `1 / sqrt(active_family_count)`.
- This preserves the ARC2 gain while making the mechanism easier to defend as unsupervised, adaptive family calibration rather than tuned global blending.
- Remaining risks are unchanged from ARC2: sparse infrastructure roots and delay/partition-like cases where propagation-shaped evidence can be causal rather than victim-only.
