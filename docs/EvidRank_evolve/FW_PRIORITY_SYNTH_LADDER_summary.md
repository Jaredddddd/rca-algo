# EvidenceRank FW_PRIORITY_SYNTH_LADDER Summary

- Created: 2026-06-04T02:03:07+08:00
...
- This version is ranking-equivalent to `FW_PRIORITY_PRIOR`.
- General mechanism: keep feature knowledge as ordinal SRE diagnostic priority and synthesize the nonlinear severity ladder from the priority-tier structure, instead of exposing exact-looking float weights as the algorithm interface.
- Direct generic ladders such as `0,1,1,1,2,5,10,15` are not safe as linear scorer weights; they should be described as ordinal IDs unless passed through the synthesized severity curve.
